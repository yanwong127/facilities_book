...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    $expiredSql = "UPDATE place_appointment SET status=:expiredStatus WHERE NOW() >= CONCAT(booking_date, ' ', end_time) AND status != :expiredStatus";
    ...
    $expiredNowSql = "UPDATE place_appointment SET status=:expiredNowStatus WHERE NOW() >= CONCAT(booking_date, ' ', end_time) AND status != :expiredNowStatus";
