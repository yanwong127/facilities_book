...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_REQUEST['reid'])) {
        ...
        $sql = "UPDATE item_appointment SET status=:status WHERE itembook_id=:reid";
        $query = $dbh->prepare($sql);
        $query->bindParam(':status', $status, PDO::PARAM_STR);
        $query->bindParam(':reid', $reid, PDO::PARAM_INT);
        $query->execute();

        $msg = "Booking Marked as Returned";
    }
    ?>
<!doctype html>
...
<head>
	...
	<title>SBooking |Admin Manage Item Booking </title>
                                            ...
                                            if ($query->rowCount() > 0) {
                                                foreach ($results as $result) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="booking_item.php?aeid=<?php echo htmlentities($result->itembook_id); ?>" onclick="return confirm('Do you really want to Confirm this booking')"> Confirm</a> /
                                                        ...
                                                        <td>
                                                            <a href="booking_item.php?reid=<?php echo htmlentities($result->itembook_id); ?>" onclick="return confirm('Do you really want to Mark this as Returned')"> Returned</a>
                                                        </td>
                                                    </tr>
                                            <?php $cnt = $cnt + 1;
                                                }
